...
def upload_image():
    ...
    with open(os.path.join('static/', filename), 'wb') as f: #wb: for converting into binary
    	f.write(file_data)
        # ends here
...
def make_grayscale(input_image):
	...
	converted_gray_img = cv2.cvtColor(decode_array_to_img, cv2.COLOR_RGB2GRAY) #cv2:converting into black and white
	status, output_image = cv2.imencode(' .PNG', converted_gray_img) #imencode: incoding image
	print('status: ', status)
